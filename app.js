...
async function renderOrgReservations(container) {
  ...
  let tableRows = '';
  if (sorted.length === 0) {
    tableRows = '<tr><td colspan="7" style="text-align:center; padding:40px;">' + window.UI.emptyState('fa-calendar-check', 'لا توجد حجوزات', 'ابدأ ببناء خطة زفاف وحجزها') + '</td></tr>';
  } else {
    ...
    '<div class="animate-in">' +
      ...
      '<div class="card" style="overflow-x:auto;">' +
        '<table class="data-table">' +
          '<thead><tr><th>رقم</th><th>العميل</th><th>التاريخ</th><th>الحضور</th><th>الخدمات</th><th>التكلفة</th><th>الحالة</th></tr></thead>' +
...
async function renderVendorLayout() {
  ...
  document.getElementById('btn-logout').addEventListener('click', () => { currentUser = null; render(); });
  document.getElementById('btn-theme').addEventListener('click', toggleTheme);
...
window.db.init().then(() => {
  ...
  console.error(e);
  app.innerHTML = '<div style="padding:60px; text-align:center;"><i class="fa-solid fa-triangle-exclamation" style="font-size:3rem; color:var(--danger); margin-bottom:16px;"></i><h2>خطأ في تحميل قاعدة البيانات</h2><p style="margin-top:8px; color:var(--text-muted);">حاول تحديث الصفحة أو مسح بيانات المتصفح</p></div>';
});
